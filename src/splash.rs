...
fn write_lists(mut out: impl Write, talks: &[Talk], kind: ListKind) -> Result<()> {
    let entries = talks
        .into_iter()
        .filter(|t| kind == ListKind::Recent || t.transcript.is_some())
        .take(5);
    ...
    for entry in entries {
        writeln!(
            &mut out,
            "            <li>{} — {}</li>",
            entry.date.format("%B %e, %Y").to_string(),
            entry.title
        )
